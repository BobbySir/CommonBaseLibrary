...
/**
 * Created by：bobby on 2021-08-18 15:19.
 * Describe：
 */
public class CalendarReminderUtils {
    ...
    private static String CALENDARS_NAME = "bobby";
    private static String CALENDARS_ACCOUNT_NAME = "bobby@test.com";
    private static String CALENDARS_ACCOUNT_TYPE = "com.android.bobby";
    private static String CALENDARS_DISPLAY_NAME = "bobby账户";
    ...
    public static void addCalendarEvent(Context context, String title, String description,
                                        long beginTimeMillis, long endTimeMillis, int previousDate) {
        if (context == null) {
            return;
        }
        ...
        mCalendar.setTimeInMillis(beginTimeMillis);//设置开始时间
        long start = mCalendar.getTime().getTime();
        mCalendar.setTimeInMillis(endTimeMillis);//设置终止时间，开始时间加10分钟
        long end = mCalendar.getTime().getTime();

        //插入日程
        ContentValues event = new ContentValues();
//        event.put(CalendarContract.Events.DTSTART, beginTimeMillis);
//        event.put(CalendarContract.Events.DTEND, endTimeMillis);
        ...
        values.put(CalendarContract.Reminders.MINUTES, previousDate);// 提前previousDate分钟有提醒
//        values.put(CalendarContract.Reminders.MINUTES, previousDate * 24 * 60);// 提前previousDate天有提醒
        values.put(CalendarContract.Reminders.METHOD, CalendarContract.Reminders.METHOD_ALERT); //设置提醒的次数
